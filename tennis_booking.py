...
async def fetch_and_book_session(date: str, show: bool = False, slow_mo: int = 0, no_booking: bool = False) -> None:
    ...
    async with async_playwright() as p:
        ...
        try: 
            ...
            await page.goto(f"https://clubspark.lta.org.uk/PrioryPark2/Booking/BookByDate#?date={date}&role=guest")
...
def parse_args():
    parser = argparse.ArgumentParser(description='Book earliest available tennis court slot')
    parser.add_argument('--show', action='store_true', help='Show browser window during automation')
    parser.add_argument('--slow', type=int, default=0, help='Add delay between actions (in milliseconds)')
    parser.add_argument('--no-booking', action='store_true', help='Only check availability without making a booking')
    parser.add_argument('date', type=lambda d: datetime.strptime(d, '%Y-%m-%d').strftime('%Y-%m-%d'), help='Date in YYYY-MM-DD format')
