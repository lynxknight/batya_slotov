...
async def run_booking_task(notifier):
    ...
    preferences = slots.SlotPreference.from_preferences_json(json.load(open('booking_preferences.json')))
    ...
    message = f"Based on preferences for {weekday=}, we should try to book something! {pref}"
    logger.info(message)
    await notifier.send_message(message, disable_notification=True)
    try:
    ...
    if not result.success:
        ...
        await notifier.send_message('You can retry via /retry command')
        return
    await notifier.send_message(f"✅ Successfully booked court for {date_str} at {slots.parse_time(result.slot.start_time)}") 